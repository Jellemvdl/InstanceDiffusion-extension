...
To evaluate the LLM submodule, we used three approaches: the first approach is a multiple raters' assessment of the realism of the generated images, secondly, we score the global description to image alignment using CLIP and the third approach involves deploying a robust Vision Language Model (VLM) called CogVLM, which determines whether the generated images match the input descriptions and bounding boxes.
...
<img src="https://github.com/Jellemvdl/InstanceDiffusion-extension/blob/main/src/data/images/LLM/bbox_vs_generation.png" alt="" width="650"/>
...
Next, we focus on verifying global text-to-image alignment using the CLIP metric. This process involved computing the cosine similarity between the features of the globally image description text and the corresponding generated image. The results show an reasonable average CLIP score of 0.278 across 100 images with a standard deviation of 0.028.

This result is especially notable when compared to the somewhat lower local attribute scores for color (0.250) and texture (0.225) reported in the original paper, highlighting the LLM's capability to generate instances that align with the global image description without compromising performance.
