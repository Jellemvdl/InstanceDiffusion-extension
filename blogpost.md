...
We prompt chatgpt with this prompt:
```setup
...
"""
```
It produces the output in the format specified in the prompt:
```setup
{{
  "caption": "the caption",
  "width": 512,
...
```
We prompt CogVLM with:

```setup
Can you provide a global caption and detailed instance-level descriptions with bounding boxes for the image's major instances, including each instance's attributes and precise location with coordinates [[x1,y1,x2,y2]]?
```

We then calculate the IoU of the bounding boxes with the generated caption from CogVLM and the bounding boxes of the scene generated by ChatGPT to assess how accurately the Instance Diffusion model places instances in the specified locations. Here are some results.
