...
The prompt we used for generated the scene is located in (to `src\lib\llm_submodule\chatgpt\chatgpt_run.py`). Moreover, the prompt for generating bounding boxes over the instances that Instance Diffusion created is located in (to `src\lib\llm_submodule\cogvlm\cogvlm.py`).
